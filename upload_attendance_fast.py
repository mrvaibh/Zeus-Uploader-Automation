...
def upload_punches(IP, sensor_id, last_log):
    ...
    try:
        ...
        while start <= end:
            ...
            if timestamp_obj > last_attendance_time_obj:
                if prev_timestamp_obj > last_attendance_time_obj:
                    end = pointer - 1
                else:
                    break
            elif timestamp_obj <= last_attendance_time_obj:
                if next_timestamp_obj < last_attendance_time_obj:
                    start = pointer + 1
                else:
